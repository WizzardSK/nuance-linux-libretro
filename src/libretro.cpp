...
static void context_reset(void)
{
    ...
    // Per libretro docs, when context_reset fires the GL state is fresh —
    // any shader/texture handles we created in a previous context are gone
    // (RA may have torn down the old context, e.g. on a fullscreen toggle).
    // Force the video layer to re-create them on the next RenderVideo.
...
void retro_run(void)
{
    ...
    // the audio path was generalized to miniaudio (upstream commit 65b3f833).
