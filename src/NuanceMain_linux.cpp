...
int main(int argc, char* argv[])
{
  ...
  while (!bQuit)
  {
    ...
    while (bRun && !nuonEnv.trigger_render_video)
    {
      ...
      // NUANCE_WORKER_BRIDGE=1 — host-side mock for MPE2's bit-4 ack.
      //
      // IS3 LOADING-screen freeze chain (see [[nuance-is3-2026-05-23]]):
      //   MPE3 sends $1D packet → MPE0 receives → MPE0 calls WriteToBus
      //   ($20301B40) which programs MPE2's slot at *0x20500500/0600 then
      //   spinwaits at $20301BBE on bit 4 of that slot. MPE2 is `go=0`
      //   (halted with interrupts disabled, per worker-halt memo) and
      //   never clears the bit → MPE0 forever → MPE3 forever.
      //
      // Bypass: continuously clear bit 4 in MPE2's polled slot regardless
      // of MPE0's PC. MPE0 spends most of its time halted (`go=0` per
      // MPESTATE) — polling MPE0's PC catches it in the halt state and
      // misses the brief spinwait window. By pre-clearing the bit, when
      // MPE0 wakes from halt and enters $20301BBE, the bit is already
      // cleared and MPE0 exits immediately. Same approach as PIN_SLOT.
      // TEMP DIAG: track three MPE0 dispatcher pointer slots referenced
      // by the commrecv ISR ($20301BE2 dispatcher).
      //   $20101C0C — general subcode dispatch (jmp (r3) for any subcode)
      //   $20101004 — $C0 subcode handler (jmp (r2))
      //   $20101000 — bit-27 handler (jsr (r1))
      {
        static uint32 s_last[3] = {0xFFFFFFFFu, 0xFFFFFFFFu, 0xFFFFFFFFu};
        static const uint32 addrs[3] = {0x20101C0Cu, 0x20101004u, 0x20101000u};
        for (int k = 0; k < 3; k++) {
          if (uint32* p = (uint32*)nuonEnv.GetPointerToMemory(0, addrs[k], false)) {
            const uint32 v = SwapBytes(*p);
            if (v != s_last[k]) {
              s_last[k] = v;
              fprintf(stderr, "[DISP-PTR] *0x%08X (MPE0) = 0x%08X\n", addrs[k], v);
            }
          }
        }
      }
      {
        static int s_wb_inited = 0; static int s_wb_on = 0;
        if (!s_wb_inited) { s_wb_inited = 1; s_wb_on = getenv("NUANCE_WORKER_BRIDGE") ? 1 : 0; }
        if (s_wb_on) {
          // MPE0 spinwait at $20301BBE: `ld_s (r5), r4; bits #4, >>#0, r4;
          // bra ne, $20301BBE`. bits #4 extracts LOW 4 bits — spins while
          // any of bits 0..3 of *r5 is set.
          static uint64 s_wb_log = 0;
          for (uint32 addr : {0x20500500u, 0x20500600u}) {
            if (uint32* p = (uint32*)nuonEnv.GetPointerToMemory(2, addr, false)) {
              const uint32 v = SwapBytes(*p);
              if (v & 0xFu) {
                *p = SwapBytes(v & ~0xFu);
                s_wb_log++;
                if (s_wb_log <= 10 || (s_wb_log % 1000) == 0)
                  fprintf(stderr, "[WORKER-BRIDGE #%llu] cleared low-nibble at *0x%08X (was 0x%08X)\n",
                          (unsigned long long)s_wb_log, addr, v);
              }
            }
          }
        }
      ...
      if ((cycles % 500) == 0)
      {
        ...
        // NUANCE_PIN_SLOT=<hex_addr>:<hex_value>[,<addr2>:<val2>,...]: pin
        // up to 8 system-DRAM slots to values every emulator main-loop tick.
        // Workaround for missing producer-side fires; the T3K level-load
        // freeze uses NUANCE_PIN_SLOT=0x806B9990:0x1 to break the MPE3
        // spinwait at $80022380. Multiple slots can extend the workaround
        // (e.g. level-transition path also gates on $806B997C).
        {
          static int s_pin_inited = 0;
          static uint32 s_pin_addr[8] = {0};
          static uint32 s_pin_value[8] = {0};
          static int s_pin_count = 0;
          if (!s_pin_inited) {
            s_pin_inited = 1;
            if (const char* spec = getenv("NUANCE_PIN_SLOT")) {
              std::string s = spec;
              s += ',';
              std::string tok;
              for (char c : s) {
                if (c == ',') {
                  if (!tok.empty() && s_pin_count < 8) {
                    uint32 a = 0, v = 0;
                    if (sscanf(tok.c_str(), "%x:%x", &a, &v) == 2 && a >= 0x80000000) {
                      s_pin_addr[s_pin_count] = a;
                      s_pin_value[s_pin_count] = v;
                      fprintf(stderr, "[PIN-SLOT] #%d forcing *0x%08X = 0x%08X every tick\n",
                              s_pin_count, a, v);
                      s_pin_count++;
                    }
                    tok.clear();
          ...
          for (int i = 0; i < s_pin_count; i++) {
            if (uint32* p = (uint32*)nuonEnv.GetPointerToMemory(3, s_pin_addr[i]))
              *p = SwapBytes(s_pin_value[i]);
          }
        }
