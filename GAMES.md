...
| 7 | Iron Soldier 3 | 🔴 Stalls post-menu (regression) | `NUANCE_MPE_RATIO=1:20:20:1 NUANCE_FORCE_AUDIOCNT=1` | **2026-05-15 update — earlier "15-asset stall" no longer reproduces in 4 consecutive runs (v1..v7).** Boot sequence: logo1 → logo2 → titel → intro → menu MPX play normally, 18 `MediaOpen`s on programs.dat (incl. 3 post-menu modules loaded from new callers rz=0x8022DDA8 / 0x800195E8 / 0x80204908), then **no further file I/O**. Result table at `0x800A596C` stays at sentinel `12101210` (asset-load wrapper at `0x8008C460` never reached — its IRAM region is zero-filled). Screen confirmed via spectacle screenshots: logo at t=60s, orange mech + loading bar at t=180s, **solid black 12 KB PNG from t=240s onwards**. MPE3 hot spot is a tight per-pixel render loop at `0x80217780..0x802177AE` (ld_s / asl / ld_b / rot / lsr — byte-LUT pixel fetch), called with `rz=0x802169F2`. All four MPEs alive (`NUANCE_LOG_MPESTATE=15` confirms MPE0 minibios oscillating between go=0 wait and go=1 active at 0x20301xxx PCs; MPE1/2 active in minibiosX dispatcher 0x20300xxx; MPE3 in main DRAM). **New stall signature at t≥195 s**: MPE3 parks at BIOS PC `0x80760388` with `commctl=0x2` (`COMM_RECV_BUFFER_FULL_BIT` permanently set) — packet sits in MPE3's recv buffer undrained, workers idle at `0x20300436`. The earlier "tex11 stall" documented in `nuance-is3-loading-stall-2026-05-14.md` belonged to a different boot path that current runs don't take. Pressing A throughout cutscenes (`NUANCE_BTN_QUEUE`) does not help. New diagnostic knob `NUANCE_LOG_MPESTATE=<sec>` added today logs go/pcexec/intsrc/commctl for all 4 MPEs. |
