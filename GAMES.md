...
| 7 | Iron Soldier 3 | 🔴 Stalls at level-select auto-attract | `NUANCE_MPE_RATIO=1:20:20:1 NUANCE_FORCE_AUDIOCNT=1` | **2026-05-16 update — state machine DOES advance naturally**: `*0x8002125C` evolves `0`(fmv.run, t≤150s) → `0x64`(menu.run, t≤240s) → **`0x66`(levelsel.run, t≥270s, persistent)**. The previously-reported `0x80760388 BIOS-handler stall` was a sampling artifact — MPE3 IS in module code at all hot times; sampling caught the high-frequency timer-interrupt handler entry by coincidence. The real stall is `levelsel.run` waiting at its `state=0x66` switch-table handler. Controller is properly initialized (`controller[1].config=0x4000000F` = STATUS_BIT + properties set), `ControllerInitialize` (slot 6) + `DeviceDetect` (slot 122) confirmed via `NUANCE_LOG_BIOS`. `NUANCE_BTN_QUEUE` writes ARE observable: `controller[1].buttons=0x4000` (A button mask) confirmed in `NUANCE_DUMP_MEM` snapshot during state=0x66. **Holding A for 10+ seconds does not advance state** — levelsel.run is NOT waiting for button input, it's waiting for the **auto-attract timeout counter** to decrement (which never ticks down in our emulator). Asset-load wrapper at `0x8008C460` is never loaded because ismerlin.run hasn't been requested yet. Next investigation: which timer/field-counter does levelsel.run decrement, and why our 60Hz video tick doesn't trigger it. Full state-evolution trace in `nuance-is3-stall-2026-05-16.md`. |
